# bj11musicimport { useState, useRef, useEffect } from "react";

const sessions = [
  {
    title: "Sunday Sunset Set",
    length: "1:04:32",
    platform: "SoundCloud",
    artwork: "https://images.unsplash.com/photo-1506157786151-b8491531f063",
    audio: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3",
  },
  {
    title: "Midnight Balcony Mix",
    length: "58:21",
    platform: "Mixcloud",
    artwork: "https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f",
    audio: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3",
  },
  {
    title: "Late Night Drift",
    length: "1:12:10",
    platform: "YouTube",
    artwork: "https://images.unsplash.com/photo-1511379938547-c1f69419868d",
    audio: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3",
  },
];

const photos = [
  "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee",
  "https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3",
  "https://images.unsplash.com/photo-1507874457470-272b3c8d8ee2",
  "https://images.unsplash.com/photo-1516450360452-9312f5e86fc7",
  "https://images.unsplash.com/photo-1487180144351-b8472da7d491",
];

export default function DJLanding() {
  const [current, setCurrent] = useState(null);
  const [playing, setPlaying] = useState(false);
  const audioRef = useRef(null);

  const playSession = (session) => {
    setCurrent(session);
    setPlaying(true);
  };

  useEffect(() => {
    if (!audioRef.current) return;
    if (playing) audioRef.current.play();
  }, [current]);

  const togglePlay = () => {
    if (!audioRef.current) return;

    if (playing) {
      audioRef.current.pause();
      setPlaying(false);
    } else {
      audioRef.current.play();
      setPlaying(true);
    }
  };

  return (
    <div className="bg-[#0E0E0E] text-[#EDEDED] min-h-screen font-sans">
      <header className="fixed top-0 left-0 right-0 backdrop-blur bg-black/40 z-20">
        <div className="max-w-6xl mx-auto flex justify-between p-5">
          <div className="font-semibold tracking-widest">B.J11</div>
          <nav className="space-x-6 text-sm">
            <a href="#sessions" className="hover:text-[#9BE7D8]">Sessions</a>
            <a href="#photos" className="hover:text-[#9BE7D8]">Photos</a>
            <a href="#about" className="hover:text-[#9BE7D8]">About</a>
            <a href="#contact" className="hover:text-[#9BE7D8]">Contact</a>
          </nav>
        </div>
      </header>

      <section className="h-screen flex items-center justify-center text-center relative overflow-hidden">
        <div className="absolute inset-0 opacity-30 bg-gradient-to-br from-[#9BE7D8] via-transparent to-[#F2C6FF] animate-pulse" />

        <div className="relative z-10">
          <h1 className="text-5xl font-semibold mb-4">B.J11</h1>
          <p className="text-[#8C8C8C] mb-6">Ambient / Chill House</p>

          <button
            onClick={() => playSession(sessions[0])}
            className="px-6 py-3 bg-[#171717] border border-[#9BE7D8]/40 rounded-full hover:shadow-[0_0_20px_rgba(155,231,216,0.4)] transition"
          >
            Play Latest Session
          </button>
        </div>
      </section>

      <section id="sessions" className="max-w-6xl mx-auto px-6 py-24">
        <h2 className="text-3xl mb-10">Recent Sessions</h2>

        <div className="grid md:grid-cols-3 gap-8">
          {sessions.map((s, i) => (
            <div
              key={i}
              className="bg-[#171717] rounded-xl overflow-hidden hover:scale-[1.02] hover:shadow-[0_0_20px_rgba(155,231,216,0.15)] transition"
            >
              <img src={s.artwork} className="h-48 w-full object-cover" />

              <div className="p-5">
                <h3 className="mb-2">{s.title}</h3>

                <p className="text-sm text-[#8C8C8C] mb-4">
                  {s.length} • {s.platform}
                </p>

                <button
                  onClick={() => playSession(s)}
                  className="text-sm px-4 py-2 border border-[#9BE7D8]/40 rounded-full hover:bg-[#9BE7D8]/10"
                >
                  Play
                </button>
              </div>
            </div>
          ))}
        </div>
      </section>

      <section id="photos" className="max-w-6xl mx-auto px-6 pb-24">
        <h2 className="text-3xl mb-10">Atmosphere</h2>

        <div className="grid md:grid-cols-3 gap-4">
          {photos.map((p, i) => (
            <div
              key={i}
              className="overflow-hidden rounded-lg group cursor-pointer"
            >
              <img
                src={p}
                className="group-hover:scale-110 transition duration-700"
              />
            </div>
          ))}
        </div>
      </section>

      <section
        id="about"
        className="max-w-5xl mx-auto px-6 py-24 grid md:grid-cols-2 gap-12 items-center"
      >
        <img
          src="https://images.unsplash.com/photo-1521335629791-ce4aec67dd53"
          className="rounded-xl"
        />

        <div>
          <h2 className="text-3xl mb-6">About</h2>

          <p className="text-[#8C8C8C] leading-relaxed">
            Blending warm pads, slow grooves, and melodic textures, B.J11
            builds long-form sets designed for late evenings and sunrise
            sessions. Each mix moves through deep house, ambient house, and
            downtempo rhythms recorded live.
          </p>
        </div>
      </section>

      <section id="contact" className="text-center pb-24">
        <h2 className="text-3xl mb-6">Bookings</h2>

        <a
          href="mailto:email@domain.com"
          className="text-[#9BE7D8] hover:drop-shadow-[0_0_10px_rgba(155,231,216,0.6)]"
        >
          email@domain.com
        </a>
      </section>

      {current && (
        <div className="fixed bottom-6 right-6 bg-[#171717]/80 backdrop-blur p-4 rounded-xl w-80 shadow-lg">
          <div className="text-sm mb-2">{current.title}</div>

          <div className="flex items-center justify-between">
            <button
              onClick={togglePlay}
              className="px-4 py-2 border border-[#9BE7D8]/40 rounded-full"
            >
              {playing ? "Pause" : "Play"}
            </button>

            <span className="text-xs text-[#8C8C8C]">{current.platform}</span>
          </div>

          <audio ref={audioRef} src={current.audio} />
        </div>
      )}

      <footer className="text-center py-12 text-[#8C8C8C] text-sm">
        <div className="space-x-6 mb-3">
          <a href="#">SoundCloud</a>
          <a href="#">Mixcloud</a>
          <a href="#">Instagram</a>
        </div>

        <div>© 2026 B.J11</div>
      </footer>
    </div>
  );
}
